...
const barOption: any = {
  ...
  xAxis: {
    ...
    splitLine: { show: true, lineStyle: { color: 'rgba(0,184,255,0.5)' } },
  ...
  yAxis: [{
    ...
    axisLine: { show: true, lineStyle: { color: '#00B8FF' } },
  }, {
    name: '单位（亿元）',
    axisTick: { show: false },
...
const enterpriseProgressRef = ref()
let enterpriseProgressChart: EChartsType | null = null
const initChart = () => {
  if (!enterpriseProgressChart)
    enterpriseProgressChart = eCharts.init(enterpriseProgressRef.value!)
...
const initEnterpriseProgressChart = () => {
  initChart()

  barOption.yAxis[0].data = prop.enterpriseProgressProp?.map(e => e?.['数据'])
  const valueTemp: any = prop.enterpriseProgressProp?.map(e => e?.['值1'])
  barOption.series[0].data = valueTemp
  barOption.series[1].data = valueTemp
  barOption.series[2].data = valueTemp

  const maximum = Math.max(...valueTemp)
  const maximumList: Array<number | { type: string }> = []
  for (let index = 0; index < valueTemp.length; index++)
    maximumList.push(maximum)

  barOption.series[3].data = maximumList

  enterpriseProgressChart?.setOption(barOption)
}

watch(() => prop.enterpriseProgressProp, (val) => {
  initEnterpriseProgressChart()
})
</script>
